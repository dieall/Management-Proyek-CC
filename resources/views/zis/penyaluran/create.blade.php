...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Input Penyaluran ZIS</h1>
</div>

<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary">Form Penyaluran</h6>
    </div>
    <div class="card-body">
                ...
                <select class="form-control @error('id_zis') is-invalid @enderror" 
                    ...
                    @foreach($zisMasuk as $zis)
                        <option value="{{ $zis->id_zis }}" 
                            data-sisa="{{ $zis->jumlah - $zis->penyaluran->sum('jumlah') }}"
                            {{ old('id_zis', request('id_zis')) == $zis->id_zis ? 'selected' : '' }}>
                            {{ $zis->muzakki->nama ?? '-' }} - {{ ucfirst($zis->jenis_zis) }} - 
                            Rp {{ number_format($zis->jumlah, 0, ',', '.') }} 
                            (Sisa: Rp {{ number_format($zis->jumlah - $zis->penyaluran->sum('jumlah'), 0, ',', '.') }})
                        </option>
                    @endforeach
                </select>
                @error('id_zis')
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('id_mustahik') is-invalid @enderror" 
                    ...
                    @foreach($mustahiks as $mustahik)
                        <option value="{{ $mustahik->id_mustahik }}" {{ old('id_mustahik') == $mustahik->id_mustahik ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control @error('tgl_salur') is-invalid @enderror" 
                               id="tgl_salur" name="tgl_salur" value="{{ old('tgl_salur', date('Y-m-d')) }}" required>
                        @error('tgl_salur')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    document.getElementById('id_zis').addEventListener('change', function() {
        ...
        if (sisa) {
            ...
            document.getElementById('jumlah').setAttribute('max', sisa);
        } else {
    ...
    // Trigger on page load if id_zis is selected

...
<div class="row">
    <div class="col-lg-9 mx-auto">
        {{-- Header Card --}}
        <div class="card mb-4 border-0" style="background: linear-gradient(135deg, #ff9800 0%, #f57c00 100%); color: white; border-radius: 12px;">
            <div class="card-body p-4">
                <div class="d-flex align-items-center">
                    ...
                    <div class="ms-4">
                        <h4 class="mb-1">
                            <i class="fas fa-arrow-down me-2"></i>Catat Penyaluran ZIS
                        </h4>
                        <p class="small mb-0 opacity-90">Distribusikan dana ZIS kepada mustahik yang berhak</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- Form Card --}}
        <form action="{{ route('admin.penyaluran.store') }}" method="POST" class="needs-validation" novalidate>
            @csrf

            {{-- SECTION 1: SUMBER DANA ZIS --}}
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-light border-bottom" style="border-radius: 8px 8px 0 0;">
                    <h6 class="mb-0 fw-bold" style="color: #ff9800;">
                        <i class="fas fa-step-backward me-2"></i>Pilih Sumber Dana
                    </h6>
                </div>
                <div class="card-body p-4">
                    <div class="mb-4">
                        ...
                        <p class="text-muted small mb-3">
                            <i class="fas fa-info-circle me-1"></i>Pilih sumber dana ZIS yang akan disalurkan
                        </p>
                        <select id="id_zis" name="id_zis" required class="form-select form-select-lg" onchange="updateZisInfo()" style="border-radius: 8px; border: 2px solid #e0e0e0;">
                            ...
                            @foreach($zismasuk as $zis)
                                ...
                                <option value="{{ $zis->id_zis }}" 
                                    ...
                                    {{ old('id_zis') == $zis->id_zis ? 'selected' : '' }}>
            ...
            <div id="infoBox" class="card mb-4 border-0 d-none" style="background: linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%); border-radius: 8px;">
                <div class="card-body p-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="p-3" style="background: white; border-radius: 8px; border-left: 4px solid #ff9800;">
            ...
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-light border-bottom" style="border-radius: 8px 8px 0 0;">
                    <h6 class="mb-0 fw-bold" style="color: #ff9800;">
                        <i class="fas fa-step-forward me-2"></i>Pilih Penerima & Tanggal
                    </h6>
                </div>
                <div class="card-body p-4">
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <p class="text-muted small mb-3">Pilih mustahik yang akan menerima dana</p>
            ...
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-light border-bottom" style="border-radius: 8px 8px 0 0;">
                    <h6 class="mb-0 fw-bold" style="color: #ff9800;">
                        <i class="fas fa-coins me-2"></i>Tentukan Nominal Penyaluran
                    </h6>
                </div>
                <div class="card-body p-4">
                    ...
                    <p class="text-muted small mb-3">Nominal yang akan disalurkan (tidak boleh melebihi sisa dana)</p>
                    
                    <div class="input-group input-group-lg mb-3" style="border-radius: 8px; overflow: hidden;">
                        <span class="input-group-text fw-bold" style="background-color: #ff9800; color: white; border: none;">Rp</span>
                        <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah') }}" 
                            ...
                            oninput="validateJumlah()" style="border: 2px solid #e0e0e0; font-size: 1.1rem;">
                    </div>

                    {{-- Warning Nominal Berlebih --}}
                    <div id="jumlahWarning" class="alert alert-danger alert-sm py-2 px-3 d-none" style="border-radius: 6px;">
                        <i class="fas fa-exclamation-triangle me-2"></i>
                        <strong>Nominal melebihi sisa dana!</strong> Silakan kurangi jumlahnya.
                    </div>

                    {{-- Info Sisa Dana --}}
                    <div id="jumlahInfo" class="alert alert-info alert-sm py-2 px-3 d-none" style="border-radius: 6px;">
                        <i class="fas fa-info-circle me-2"></i>
                        <strong id="jumlahInfoText">Sisa setelah penyaluran: Rp 0</strong>
                    </div>
            ...
            <div class="card mb-4 border-0 shadow-sm">
                <div class="card-header bg-light border-bottom" style="border-radius: 8px 8px 0 0;">
                    <h6 class="mb-0 fw-bold" style="color: #ff9800;">
                        <i class="fas fa-note-sticky me-2"></i>Catatan (Opsional)
                    </h6>
                </div>
                <div class="card-body p-4">
                    ...
                    <p class="text-muted small mb-3">Cantumkan alasan atau keterangan penyaluran (opsional)</p>
                    <textarea id="keterangan" name="keterangan" rows="4" class="form-control" 
                        ...
                        style="border: 2px solid #e0e0e0; border-radius: 8px;">{{ old('keterangan') }}</textarea>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <div class="d-flex gap-3">
                        <button type="submit" class="btn btn-lg fw-bold flex-grow-1" style="background: linear-gradient(135deg, #ff9800 0%, #f57c00 100%); color: white; border: none; border-radius: 8px;">
                            <i class="fas fa-check-circle me-2"></i>Simpan Penyaluran
                        </button>
                        <a href="{{ route('admin.penyaluran.index') }}" class="btn btn-lg btn-light fw-bold" style="border: 2px solid #e0e0e0; border-radius: 8px;">
...
<script>
    ...
    function validateJumlah() {
        ...
        const info = document.getElementById('jumlahInfo');
        const jumlah = parseInt(jumlahInput.value) || 0;

        if (jumlah > sisaDanaGlobal && sisaDanaGlobal > 0) {
            ...
            if (sisaDanaGlobal > 0 && jumlah > 0) {
                const tersisa = sisaDanaGlobal - jumlah;
                document.getElementById('jumlahInfoText').textContent = 
                    `Sisa setelah penyaluran: Rp ${new Intl.NumberFormat('id-ID').format(tersisa)}`;

...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.penyaluran.update', $penyaluran) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select id="id_zis" name="id_zis" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($zismasuk as $zis)
                        <option value="{{ $zis->id_zis }}" {{ old('id_zis', $penyaluran->id_zis) == $zis->id_zis ? 'selected' : '' }}>
                            {{ $zis->muzakki->nama }} - Rp {{ number_format($zis->jumlah, 0, ',', '.') }}
                        </option>
                    @endforeach
                </select>
                @error('id_zis')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
            </div>
            ...
            <div>
                ...
                @error('id_mustahik')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
            </div>
            ...
            <div>
                ...
                @error('tgl_salur')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
            </div>

            <div>
                <label for="jumlah" class="block text-sm font-medium text-gray-700 mb-2">Jumlah (Rp) <span class="text-red-500">*</span></label>
                <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah', $penyaluran->jumlah) }}" min="0" step="1000" required 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
            ...
            <textarea id="keterangan" name="keterangan" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('keterangan', $penyaluran->keterangan) }}</textarea>
        </div>
        ...
        <div class="flex gap-2 pt-4 border-t">
            ...
            <a href="{{ route('admin.penyaluran.show', $penyaluran) }}" class="bg-gray-400 text-white px-6 py-2 rounded hover:bg-gray-500">

...
@section('title', 'Tambah Mustahik - Manajemen ZIS')
@section('page_title', 'Tambah Mustahik')

@section('content')
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.mustahik.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="kategori_mustahik" name="kategori_mustahik" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat }}" {{ old('kategori_mustahik') === $cat ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $cat)) }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="surat_dtks" name="surat_dtks" value="{{ old('surat_dtks') }}" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div class="flex gap-2 pt-4 border-t">
            ...
            <a href="{{ route('admin.mustahik.index') }}" class="bg-gray-400 text-white px-6 py-2 rounded hover:bg-gray-500">

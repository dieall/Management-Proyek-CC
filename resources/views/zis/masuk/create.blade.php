...
@section('title', 'Tambah ZIS Masuk - Manajemen ZIS')
@section('page_title', 'Tambah ZIS Masuk')

@section('content')
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.zis.masuk.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select id="id_muzakki" name="id_muzakki" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    @foreach($muzakki as $m)
                        <option value="{{ $m->id_muzakki }}" {{ old('id_muzakki') == $m->id_muzakki ? 'selected' : '' }}>
                            {{ $m->nama }} - {{ $m->no_hp ?? '-' }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" id="tgl_masuk" name="tgl_masuk" value="{{ old('tgl_masuk', date('Y-m-d')) }}" 
        ...
        <div class="flex gap-2 pt-4 border-t">
            ...
            <a href="{{ route('admin.zis.masuk.index') }}" class="bg-gray-400 text-white px-6 py-2 rounded hover:bg-gray-500">

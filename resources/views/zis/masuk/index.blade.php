...
@section('content')
    <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg shadow-md p-6 border-l-4 border-blue-600">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-700 mt-2">{{ $countZis }}</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-purple-700 mt-2">Rp
                            {{ $countZis > 0 ? number_format($totalZis / $countZis, 0, ',', '.') : '0' }}</p>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form method="GET" action="{{ route('admin.zis.masuk.index') }}"
                ...
                {{-- Filter Jenis ZIS --}}
                <div>
                    <label for="jenis_zis" class="block text-sm font-semibold text-gray-700 mb-2">Jenis ZIS</label>
                    <select name="jenis_zis" id="jenis_zis"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="">-- Semua Jenis --</option>
                        <option value="zakat" {{ $jenisZis == 'zakat' ? 'selected' : '' }}>Zakat</option>
                        <option value="infaq" {{ $jenisZis == 'infaq' ? 'selected' : '' }}>Infaq</option>
                        <option value="shadaqah" {{ $jenisZis == 'shadaqah' ? 'selected' : '' }}>Shadaqah</option>
                        <option value="wakaf" {{ $jenisZis == 'wakaf' ? 'selected' : '' }}>Wakaf</option>
                    </select>
                </div>

                {{-- Search Muzakki --}}
                <div>
                    <label for="search" class="block text-sm font-semibold text-gray-700 mb-2">Nama Muzakki</label>
                ...
                <div class="flex items-end">
                    <button type="submit"
                        class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition font-semibold flex items-center justify-center">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.zis.masuk.create') }}"
                        ...
                        @forelse($zisMasuk as $item)
                            <tr class="border-b hover:bg-gray-50">
                                <td class="px-4 py-2 text-sm text-gray-700">
                                    {{ ($zisMasuk->currentPage() - 1) * $zisMasuk->perPage() + $loop->iteration }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">
                                ...
                                <td class="px-4 py-2 text-center text-sm">
                                    <a href="{{ route('admin.zis.masuk.show', $item) }}"
                                        class="text-blue-600 hover:text-blue-800 mr-2">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.zis.masuk.edit', $item) }}"
                                        class="text-yellow-600 hover:text-yellow-800 mr-2">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.zis.masuk.destroy', $item) }}" method="POST"
